...
if (!function_exists('stm_taxonomy_wcmap_add_field')) {
    function stm_taxonomy_wcmap_add_field($taxonomy)
    {
        ...
        <div class="form-field">
            <label for="stm_taxonomy_wcmap_image"><?php esc_html_e('Advertising Banner'); ?></label>
        ...
        <div class="form-field">
            <label for="stm_taxonomy_wcmap_image"><?php esc_html_e('Advertising Banner Link'); ?></label>
            <div class="stm-choose-wcmap-image">
                <input
                    type="text"
    ...
    function stm_taxonomy_wcmap_edit_field( $tag, $taxonomy ) {
        ...
        <tr class="form-field">
            <th scope="row" valign="top"><label
                    for="stm_taxonomy_wcmap_image"><?php esc_html_e('Advertising Banner'); ?></label></th>
            <td>
                <div class="stm-choose-wcmap-image">
                    <input
                        type="hidden"
        ...
        <tr class="form-field">
            <th scope="row" valign="top"><label
                    for="stm_taxonomy_wcmap_link"><?php esc_html_e('Advertising Banner Link'); ?></label></th>
            <td>
...
if (!function_exists('stm_taxonomy_wcmap_image_save')) {
    function stm_taxonomy_wcmap_image_save($term_id, $tt_id)
    {
        if (isset($_POST['stm_taxonomy_wcmap_image'])) {
            update_term_meta($term_id, 'stm_wcmap_image', $_POST['stm_taxonomy_wcmap_image']);
        }

        if (isset($_POST['stm_taxonomy_wcmap_link'])) {
            update_term_meta($term_id, 'stm_banner_link', $_POST['stm_taxonomy_wcmap_link']);
        }
    }
}