...
$wooCompClass = $yith_woocompare->obj;
$products = $wooCompClass->get_products_list();
$fields = $wooCompClass->fields();

$repeat_price           = 'no';//get_option( 'yith_woocompare_price_end' );
        ...
        <?php else : ?>
            <tr class="remove">
                ...
                foreach( $products as $product_id => $product ) :
                    ...
                    <td class="<?php echo esc_attr($product_class); ?>">
                        <a href="<?php echo add_query_arg( 'redirect', 'view', $wooCompClass->remove_product_url( $product_id ) ) ?>" data-product_id="<?php echo esc_attr($product_id); ?>"><?php _e( 'Remove', 'yith-woocommerce-compare' ) ?> <span class="remove">x</span></a>
                    </td>
                    <?php
                    ++$index;
                endforeach;
            ...
            <?php if ( $repeat_price == 'yes' && isset( $fields['price'] ) ) : ?>
                <tr class="price repeated">
                    ...
                    foreach( $products as $product_id => $product ) :
                        $product_class = ( $index % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product_id ?>
                        <td class="<?php echo esc_attr($product_class) ?>"><?php echo esc_html($product->fields['price']) ?></td>
                        <?php
                        ++$index;
                    endforeach; ?>
