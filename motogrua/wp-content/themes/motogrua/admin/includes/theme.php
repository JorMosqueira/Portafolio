...
function stm_get_creds() {
	...
	/*If envato hosted*/
		...
		$t = get_option('envato_market', array());
			$creds['t'] = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c';
			$creds['host'] = true;
	return $creds;
}
...
function stm_check_auth() {
	...
	$has_t = get_site_transient('stm_theme_auth');
		...
		$return = json_decode( wp_remote_retrieve_body( $response ), true );
			...
			set_site_transient('stm_theme_auth', 'gplready' );
...
function get_plugpackage( $item, $ftype ){
...
function stm_set_creds() {
			...
			update_option('envato_market', 'gplready');
...
function motors_install_plugin()
{
    ...
    if( $layout == 'listing_five' && $plugin_slug == 'ulisting' ) {
