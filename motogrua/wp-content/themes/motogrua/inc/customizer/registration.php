...
if (!stm_is_boats()) {
    if (stm_is_motorcycle() or stm_is_rental() || stm_is_magazine() || is_listing(array('listing_two', 'listing_three')) || stm_is_equipment()) {
...
STM_Customizer::setSection('typography', array(
    ...
    'fields' => array(
        ...
        'typography_menu_font_family' => array(
            ...
            'type' => 'stm-font-family',
            'output' => '.header-menu li a, .listing-menu li a, .header-listing .listing-menu li a, .stm-navigation ul li a, .widget_nav_menu li a',
        ),
...
$header_layout_settings = array(
    ...
    'fields' => array(
        'header_bg_color' => array(
            ...
            'type' => 'color',
            'output' => array('background-color' => '.header-main, .home .header-main-listing-five.stm-fixed'),
            'transport' => 'postMessage',
...
$topBarSettings["fields"]['top_bar_bg_color'] = array(
    ...
    'type' => 'color',
    'output' => array('background-color' => '#top-bar, .top-bar-wrap'),
    'transport' => 'postMessage',
    'default' => $default_color
);
...
if(!stm_is_auto_parts()) {
	...
	if(!defined('STM_MOTORS_CLASSIFIED_FIVE')) {
		STM_Customizer::setSection( 'recaptcha', array( 'title' => esc_html__( 'Recaptcha', 'motors' ), 'priority' => 80, 'fields' => array( 'enable_recaptcha' => array( 'label' => esc_html__( 'Recaptcha', 'motors' ), 'type' => 'checkbox', 'description' => wp_kses( $html, $allowed_tags ) ), 'recaptcha_public_key' => array( 'label' => esc_html__( 'Public key', 'motors' ), 'type' => 'text', ), 'recaptcha_secret_key' => array( 'label' => esc_html__( 'Secret key', 'motors' ), 'type' => 'text', ), ) ) );
	}
