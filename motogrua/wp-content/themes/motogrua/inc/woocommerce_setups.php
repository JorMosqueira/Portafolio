...
function stm_woocommerce_header_add_to_cart_fragment( $fragments )
{
	...
	if ( $cart_count == 0 ) {
		$cart_count = '';
		if ( stm_is_boats() or stm_is_dealer_two() ) {
			$cart_count = '0';
		}
	}
	ob_start();
...
if ( stm_pricing_enabled() ) {
	...
	if ( class_exists( 'STM_PostType' ) ) {
		...
		STM_PostType::addMetaBox( 'stm_pricing_plans', esc_html__( 'Pricing Plan Options (Works only with "Subscription" Product)', 'motors' ), array( 'product' ), '', '', '', array( 'fields' => array( 'stm_price_plan_quota' => array( 'label' => __( 'Price Plan Cars Quota', 'motors' ), 'type' => 'text', ), 'stm_price_plan_media_quota' => array( 'label' => __( 'Price Plan Car Images Quota', 'motors' ), 'type' => 'text', ), 'stm_price_plan_role' => array( 'label' => __( 'Price Plan User Role', 'motors' ), 'type' => 'select', 'options' => array( 'user' => __( 'User', 'motors' ), 'dealer' => __( 'Dealer', 'motors' ), ) ), ) ) );
	}
		    ...
		    if(!stm_is_multiple_plans()) {
				...
				if ( !in_array( $new_status, array( 'active', 'trial' ) ) ) {
					...
					if ( $new_status == 'active' ) {
						...
						$post_limit = stm_user_active_subscriptions( false, $subscription->user_id );
		        ...
		        $plansMeta = MultiplePlan::getPlans();

		        if ( !in_array( $new_status, array( 'active', 'trial' ) ) ) {

                    $listingIds = MultiplePlan::getListingIdsByPlanId($subscription->id);

                    if($listingIds) {
						foreach ( $listingIds as $val ) {
							$draft_post = array( 'ID' => $val->listing_id, 'post_status' => 'draft' );
							MultiplePlan::updatePlanMeta($subscription->id, $val->listing_id, 'draft');
							wp_update_post( $draft_post );
						}
					}
					...
					if($listingIds) {
						foreach ( $listingIds as $val ) {
							...
							MultiplePlan::updatePlanMeta($subscription->id, $val->listing_id, 'active');
							wp_update_post( $draft_post );
						}
					}
                }
