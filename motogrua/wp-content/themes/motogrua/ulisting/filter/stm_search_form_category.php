                ...
                foreach ($categories as $category): if ($i == 0) $data['active_tab'] = $category->term_id ?>
                    <li class="nav-item">
                        <a class="nav-link stm-cursor-pointer heading-font"
                           ...
                           data-v-on_click="set_active_tab(<?php echo esc_attr($category->term_id) ?>)"><i class="<?php echo esc_attr(stm_c_f_get_category_icon($category->term_id)); ?>"></i> <?php echo esc_html($category->name) ?></a>
                    </li>
                    <?php $i++; endforeach; ?>
            </ul>
